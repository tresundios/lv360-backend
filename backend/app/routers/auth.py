...
def register_step2(body: RegisterStep2Request, db: Session = Depends(get_db)):
    result = auth_service.register_step2(
...
def register_verify(body: RegisterVerifyRequest, db: Session = Depends(get_db)):
    return auth_service.register_verify(body.user_id, body.otp_code, db)
...
def login_2fa(body: Login2FARequest, db: Session = Depends(get_db)):
    return auth_service.login_2fa(body.user_id, body.otp_code, db)
...
def otp_resend(body: OTPResendRequest, db: Session = Depends(get_db)):
    return auth_service.resend_otp(body.user_id, body.purpose, db)
...
def refresh(body: RefreshRequest, db: Session = Depends(get_db)):
    return auth_service.refresh_tokens(body.refresh_token, db)
...
def logout(body: LogoutRequest, db: Session = Depends(get_db)):
    return auth_service.logout(body.refresh_token, db)
...
def get_invite(token: str, db: Session = Depends(get_db)):
    return auth_service.get_invitation(token, db)
...
def accept_invite(body: InviteAcceptRequest, db: Session = Depends(get_db)):
    return auth_service.accept_invitation(body.token, body.full_name, body.password, db)
...
async def google_callback(code: str = "", db: Session = Depends(get_db)):
    ...
    raise HTTPException(
        ...
        detail="Google OAuth chưa được cấu hình. Vui lòng thiết lập AUTH_GOOGLE_CLIENT_ID trong .env.local.",
    )
...
async def zalo_callback(code: str = "", db: Session = Depends(get_db)):
    ...
    raise HTTPException(
        status_code=status.HTTP_501_NOT_IMPLEMENTED,
        detail="Zalo OAuth chưa được cấu hình. Vui lòng thiết lập AUTH_ZALO_APP_ID trong .env.local.",
    )
...
def first_login_complete(
    ...
    return auth_service.complete_first_login(current_user, db)
...
def ai_abuse(
    ...
    return auth_service.record_ai_abuse(current_user, db)
